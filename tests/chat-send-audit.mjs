...
window.SillyTavern = globalThis.SillyTavern = {
  getContext: () => ({
    ...
    characters: [{ name: 'Test Character' }], chat: [], eventSource,
  }),
...
ui.currentContact = contacts[0] || created;
...
for (const method of ['addMessage', 'addMessages']) {
  ...
  storage[method] = async (...args) => {
    writes.push({ method, args: plain(args) });
...
const variants = {
  plain: '你好，我收到了。',
  jsonStringArray: '["你好，我收到了。"]',
  object: '{"content":"你好，我收到了。","type":"text"}',
  objectRole: '{"role":"assistant","content":"你好，我收到了。","type":"text"}',
  arrayObject: '[{"content":"你好，我收到了。","type":"text"}]',
  arrayRoleObject: '[{"role":"assistant","content":"你好，我收到了。","type":"text"}]',
  messagesObject: '{"messages":[{"role":"assistant","content":"你好，我收到了。","type":"text"}]}',
  tagged: '<message>你好，我收到了。</message>',
  twoLines: '你好。\n我收到了。',
};
const results = {};
...
for (const [name, response] of Object.entries(variants)) {
  writes.length = 0;
  ui.messages = [];
  ai.sendChatRequest = async () => response;
  try {
    const result = await ui.getAIResponse(`test-${name}`);
    results[name] = { result: plain(result), writes: plain(writes), uiMessages: plain(ui.messages) };
  } catch (error) {
    results[name] = { error: error?.message || String(error), stack: error?.stack?.slice(0, 800), writes: plain(writes), uiMessages: plain(ui.messages) };
  }
}
ai.sendChatRequest = originalSendChatRequest;
window.close();
throw new Error(`CHAT_FORMAT_MATRIX=${JSON.stringify(results)}`);
